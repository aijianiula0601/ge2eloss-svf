...
from dataset.test_data_load import load_multi_person_one_utterances
...
for speaker_dir in Path(speakers_dir).glob("*"):
    ...
    #     ...
    # ]
    speakers_utterance_mels = load_multi_person_one_utterances(read_dir=Path(speakers_dir), n_speaker=4,
                                                               extension="wav")

    n_speaker_utterance_embeds_cl = []
    for utterance_mel in speakers_utterance_mels:
        ...
        n_speaker_utterance_embeds_cl.append(utterance_embed)

    n_speaker_utterance_embed = np.array(n_speaker_utterance_embeds_cl)

    # 计算错误接受率
    inter_class_matrix = np.matmul(n_speaker_utterance_embed, n_speaker_utterance_embed.T)

    frp = frp_fap.fap(inter_class_matrix, threshold_p=threshold_p)

    print("fap:", frp)
    fap_cl.append(frp)

print("平均错误接受率:{}".format(np.mean(np.array(fap_cl))))
